...
(() => {
  ...
  const specialType=/A388|A124|AN12|AN22|AN124|AN225|C17|C5M|B52|B1B|TU95/i;
  const militaryWords=/MILITARY|AIR FORCE|NAVY|ARMY|LUCHTMACHT|RAF|USAF|NATO/i;
  const isMilitary=a=>Boolean(Number(a.dbFlags)&1)||militaryWords.test([a.operator,a.ownOp,a.description,a.flight].filter(Boolean).join(' '));
  const isHeavy=a=>String(a.category||'').toUpperCase()==='A5'||/A38|B74|B77|A34|A35|MD11|IL96|AN12/i.test(String(first(a.aircraft_type,a.t,a.model)||''));
  const isEmergency=a=>Boolean(emergencySquawks[String(a.squawk||'')]);
  const match=a=>{
    const alt=Number(first(a.altitude_ft,a.alt_baro)); const dist=Number(a.distance_km);
    if(Number.isFinite(dist)&&dist>state.radius)return false;
    if(Number.isFinite(alt)&&(alt<state.minAlt||alt>state.maxAlt))return false;
    if(state.filter==='heavy'&&!isHeavy(a))return false;
    if(state.filter==='helicopter'&&!isHelicopter(a))return false;
    if(state.filter==='military'&&!isMilitary(a))return false;
