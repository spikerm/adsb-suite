...
(()=>{
  ...
  let trailLayers=new Map();

  apply=function(data){
    liveSnapshot=data;
    if(!replayActive)originalApplyV10(data);
  };
  ...
  function activateView(name){
    ...
    if(name==='replay')setTimeout(()=>map.invalidateSize(),80);
  }
  button.onclick=()=>activateView('replay');

  function clearTrails(){
    trailLayers.forEach(layer=>map.removeLayer(layer));trailLayers.clear();trails.clear();
  }
