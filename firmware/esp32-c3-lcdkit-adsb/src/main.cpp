...
static AircraftDot g_parse_planes[MAX_PLANES];
...
// MQTT transport buffer. The MQTT task only assembles bytes here. JSON parsing is
// deliberately done by g_parser_task so cJSON can never overflow mqtt_task's stack.
...
static volatile bool g_mqtt_payload_ready = false;
static volatile bool g_mqtt_drop_current = false;
static TaskHandle_t g_parser_task = nullptr;
static uint32_t g_mqtt_rx_messages = 0;
static uint32_t g_mqtt_dropped_messages = 0;
...
static void handle_aircraft(const char *data, int len)
{
    ...
    memset(g_parse_planes, 0, sizeof(g_parse_planes));
    size_t count = 0;
    cJSON *item = nullptr;
    cJSON_ArrayForEach(item, root) {
        if (count >= MAX_PLANES) break;
        if (cJSON_IsObject(item)) parse_plane(item, g_parse_planes[count++]);
    }

    if (xSemaphoreTake(g_data_mutex, pdMS_TO_TICKS(100))) {
        memcpy(g_planes, g_parse_planes, sizeof(g_planes));
        g_plane_count = count;
        xSemaphoreGive(g_data_mutex);
        g_data_dirty = true;
    }
    ...
    if (count) {
        ESP_LOGI(TAG, "aircraft parsed: %u first=%s %.1fkm %.0fdeg",
                 (unsigned)count,
                 g_parse_planes[0].flight[0] ? g_parse_planes[0].flight : g_parse_planes[0].hex,
                 (double)g_parse_planes[0].distance_km, (double)g_parse_planes[0].bearing);
    } else {
...
{
    ++g_mqtt_rx_messages;
    ESP_LOGI(TAG, "parser RX #%u %s %d bytes stackfree=%u",
             (unsigned)g_mqtt_rx_messages, topic, len,
    ...
    else ESP_LOGW(TAG, "parser unknown topic: %s", topic);
}
...
static void reset_mqtt_assembly()
{
    ...
    g_mqtt_topic[0] = 0;
    g_mqtt_drop_current = false;
}

static void parser_task(void *)
{
    ESP_LOGI(TAG, "ADS-B parser task started");
    while (true) {
        ...
        reset_mqtt_assembly();
        g_mqtt_payload_ready = false;
        ESP_LOGI(TAG, "parser done stackfree=%u heap=%u",
                 (unsigned)uxTaskGetStackHighWaterMark(nullptr),
                 (unsigned)heap_caps_get_free_size(MALLOC_CAP_8BIT));
    }
}
...
static void mqtt_event(void *, esp_event_base_t, int32_t event_id, void *event_data)
{
    ...
    switch ((esp_mqtt_event_id_t)event_id) {
        ...
        case MQTT_EVENT_DISCONNECTED:
            ...
            g_data_dirty = true;
            if (!g_mqtt_payload_ready) reset_mqtt_assembly();
        ...
        case MQTT_EVENT_DATA: {
            ...
            if (g_mqtt_payload_ready) {
                if (e->current_data_offset == 0) {
                    ...
                    ESP_LOGW(TAG, "MQTT RX busy, dropping message total=%d dropped=%u",
                             e->total_data_len, (unsigned)g_mqtt_dropped_messages);
                }
                break;
            }
            ...
            if (e->current_data_offset == 0) {
                ...
                if ((size_t)g_mqtt_expected_len >= MQTT_RX_MAX) {
                    g_mqtt_drop_current = true;
            ...
            if (off + n >= MQTT_RX_MAX) {
                g_mqtt_drop_current = true;
                ESP_LOGW(TAG, "MQTT fragment overflow off=%u len=%u", (unsigned)off, (unsigned)n);
                break;
            }
            ...
            if (g_mqtt_rx_total >= (size_t)g_mqtt_expected_len) {
                ...
                g_mqtt_payload_ready = true;
                ESP_LOGI(TAG, "MQTT assembled %s %d bytes -> parser",
                         g_mqtt_topic, g_mqtt_expected_len);
                if (g_parser_task) xTaskNotifyGive(g_parser_task);
            }
            break;
        }
        default: break;
...
static void start_mqtt()
{
    ...
    if (!g_parser_task) {
        ...
        if (ok != pdPASS) {
            ESP_LOGE(TAG, "Failed to create ADS-B parser task");
        } else {
            log_heap("after parser task start");
        }
    }
}
...
static void update_ui_data(const Summary &s, const AircraftDot *planes, size_t count)
{
    ...
    snprintf(buf, sizeof(buf), "%s\nWi-Fi %s  MQTT %s\nSummary RX %ds\nAircraft RX %ds\nTargets %u\nDrops %u",
             ...
             summary_age, aircraft_age, (unsigned)count, (unsigned)g_mqtt_dropped_messages);
...
extern "C" void app_main(void)
{
    ...
    xTaskCreate(ui_task, "adsb-ui", 3584, nullptr, 4, nullptr);
