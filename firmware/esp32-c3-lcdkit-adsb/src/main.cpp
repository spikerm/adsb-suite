...
static int g_radar_range_idx = 1;
...
static void mqtt_event(void *, esp_event_base_t, int32_t event_id, void *event_data)
{
    ...
    switch ((esp_mqtt_event_id_t)event_id) {
        case MQTT_EVENT_CONNECTED: {
            ...
            ESP_LOGI(TAG, "subscribe mid=%d topic=%s", esp_mqtt_client_subscribe(g_mqtt, topic, 0), topic);
        ...
        case MQTT_EVENT_DATA: {
            ...
            if (e->current_data_offset == 0) {
                ...
                ESP_LOGI(TAG, "MQTT fragment start topic=%s total=%d first=%d", g_mqtt_topic, e->total_data_len, e->data_len);
            }
            if (g_mqtt_expected_len <= 0 || g_mqtt_topic[0] == 0) break;
...
static void start_mqtt()
{
    ...
    if (err == ESP_OK) {
        ...
        log_heap("after mqtt start failure");
    }
}
...
static lv_obj_t *plain_obj(lv_obj_t *parent) { lv_obj_t *o = lv_obj_create(parent); lv_obj_remove_style_all(o); return o; }
static lv_obj_t *label(lv_obj_t *parent, const char *text, int y, uint32_t color = 0xffffff)
{
    lv_obj_t *o = lv_label_create(parent); lv_label_set_text(o, text ? text : "");
    lv_obj_set_style_text_color(o, lv_color_hex(color), 0); lv_obj_align(o, LV_ALIGN_TOP_MID, 0, y); return o;
}
static lv_obj_t *make_page(lv_obj_t *screen)
{
    lv_obj_t *p = plain_obj(screen); lv_obj_set_size(p, 240, 240); lv_obj_align(p, LV_ALIGN_CENTER, 0, 0); return p;
}
static void make_ring(lv_obj_t *parent, int diameter, uint32_t color, int width = 1)
{
    lv_obj_t *r = plain_obj(parent); lv_obj_set_size(r, diameter, diameter); lv_obj_set_style_radius(r, LV_RADIUS_CIRCLE, 0);
    lv_obj_set_style_bg_opa(r, LV_OPA_TRANSP, 0); lv_obj_set_style_border_width(r, width, 0);
    lv_obj_set_style_border_color(r, lv_color_hex(color), 0); lv_obj_align(r, LV_ALIGN_CENTER, 0, 0);
}
static lv_obj_t *make_line(lv_obj_t *parent, uint32_t color, int width)
{
    lv_obj_t *line = lv_line_create(parent); lv_obj_set_style_line_color(line, lv_color_hex(color), 0);
    lv_obj_set_style_line_width(line, width, 0); lv_obj_set_style_line_rounded(line, true, 0); return line;
}

static void page_focus_cb(lv_event_t *e)
{
    int page = (int)(intptr_t)lv_event_get_user_data(e); if (page < 0 || page > 3) return;
    g_page = page; g_data_dirty = true; ESP_LOGI(TAG, "page -> %d", g_page);
}
static void page_click_cb(lv_event_t *e)
{
    int page = (int)(intptr_t)lv_event_get_user_data(e);
    if (page == 1) { g_radar_range_idx = (g_radar_range_idx + 1) % 5; ESP_LOGI(TAG, "radar range -> %d km", g_radar_ranges[g_radar_range_idx]); g_data_dirty = true; }
    else if (g_nav_group) lv_group_focus_next(g_nav_group);
}
...
static void build_ui()
{
    ...
    g_overview_info = label(g_pages[0], "POS 0\nMSG/S 0\nMAX 0 km", 82); lv_obj_set_style_text_align(g_overview_info, LV_TEXT_ALIGN_CENTER, 0);
    g_overview_near = label(g_pages[0], "NEAREST ---", 150); lv_obj_set_style_text_align(g_overview_near, LV_TEXT_ALIGN_CENTER, 0);

    g_pages[1] = make_page(screen); lv_obj_set_style_bg_color(g_pages[1], lv_color_hex(RADAR_BG), 0); lv_obj_set_style_bg_opa(g_pages[1], LV_OPA_COVER, 0);
    make_ring(g_pages[1], 236, RADAR_GREEN_MED, 2); make_ring(g_pages[1], 180, RADAR_GREEN_DIM); make_ring(g_pages[1], 124, RADAR_GREEN_DIM); make_ring(g_pages[1], 68, RADAR_GREEN_DIM);
    static lv_point_precise_t hpts[2] = {{8,120},{232,120}}; static lv_point_precise_t vpts[2] = {{120,8},{120,232}};
    ...
    for (int i = 0; i < 12; ++i) {
        ...
        g_sweep_lines[i] = make_line(g_pages[1], c, i == 0 ? 2 : 1); lv_line_set_points(g_sweep_lines[i], g_sweep_points[i], 2);
        int opa = 255 - i * 19; if (opa < 35) opa = 35; lv_obj_set_style_line_opa(g_sweep_lines[i], (lv_opa_t)opa, 0);
    }
    for (size_t i = 0; i < MAX_PLANES; ++i) {
        lv_obj_t *d = plain_obj(g_pages[1]); lv_obj_set_size(d, 8, 8); lv_obj_set_style_radius(d, LV_RADIUS_CIRCLE, 0);
        lv_obj_set_style_bg_color(d, lv_color_hex(RADAR_GREEN), 0); lv_obj_set_style_bg_opa(d, LV_OPA_COVER, 0);
        lv_obj_set_style_border_width(d, 1, 0); lv_obj_set_style_border_color(d, lv_color_hex(0xb0ffbd), 0); lv_obj_add_flag(d, LV_OBJ_FLAG_HIDDEN); g_radar_targets[i] = d;
    }
    g_radar_title = label(g_pages[1], "PPI 50 km", 8, RADAR_GREEN);
    g_radar_count = label(g_pages[1], "0 targets", 216, RADAR_GREEN_MED);
    lv_obj_t *north = lv_label_create(g_pages[1]); lv_label_set_text(north, "N"); lv_obj_set_style_text_color(north, lv_color_hex(RADAR_GREEN), 0); lv_obj_set_pos(north, 116, 25);

    g_pages[2] = make_page(screen); label(g_pages[2], "NEAREST", 10, 0x00d7ff); g_nearest_info = label(g_pages[2], "---", 52); lv_obj_set_style_text_align(g_nearest_info, LV_TEXT_ALIGN_CENTER, 0);
    g_pages[3] = make_page(screen); label(g_pages[3], "RECEIVER", 10, 0x00d7ff); g_status_info = label(g_pages[3], "SOURCE OFFLINE", 58); lv_obj_set_style_text_align(g_status_info, LV_TEXT_ALIGN_CENTER, 0);

    g_alert_label = lv_label_create(screen); lv_label_set_text(g_alert_label, ""); lv_obj_set_style_text_color(g_alert_label, lv_color_white(), 0);
    lv_obj_set_style_bg_color(g_alert_label, lv_color_hex(0xc00000), 0); lv_obj_set_style_bg_opa(g_alert_label, LV_OPA_COVER, 0);
    lv_obj_set_style_pad_all(g_alert_label, 5, 0); lv_obj_align(g_alert_label, LV_ALIGN_BOTTOM_MID, 0, -8); lv_obj_add_flag(g_alert_label, LV_OBJ_FLAG_HIDDEN);

    lv_indev_t *indev = bsp_display_get_input_dev();
    if (indev && lv_indev_get_type(indev) == LV_INDEV_TYPE_ENCODER) {
        g_nav_group = lv_group_create();
        for (int i = 0; i < 4; ++i) {
            lv_obj_t *btn = lv_button_create(screen); lv_obj_set_size(btn, 1, 1); lv_obj_set_pos(btn, -10, -10);
            lv_obj_set_style_bg_opa(btn, LV_OPA_TRANSP, 0); lv_obj_set_style_border_width(btn, 0, 0); lv_obj_set_style_outline_width(btn, 0, 0);
            lv_obj_add_event_cb(btn, page_focus_cb, LV_EVENT_FOCUSED, (void *)(intptr_t)i); lv_obj_add_event_cb(btn, page_click_cb, LV_EVENT_CLICKED, (void *)(intptr_t)i);
            lv_group_add_obj(g_nav_group, btn); g_nav_buttons[i] = btn;
        }
        lv_indev_set_group(indev, g_nav_group); lv_group_focus_obj(g_nav_buttons[0]); ESP_LOGI(TAG, "Using BSP encoder input device");
    }
...
{
    for (int i = 0; i < 4; ++i) { if (i == page) lv_obj_remove_flag(g_pages[i], LV_OBJ_FLAG_HIDDEN); else lv_obj_add_flag(g_pages[i], LV_OBJ_FLAG_HIDDEN); }
}
static float angle_delta(float a, float b) { float d = fmodf(a - b + 540.0f, 360.0f) - 180.0f; return fabsf(d); }
static void update_sweep()
{
    for (int i = 0; i < 12; ++i) {
        float a = (float)(g_sweep_deg - i * 3 - 90) * PI_F / 180.0f;
        g_sweep_points[i][0].x = RADAR_CX; g_sweep_points[i][0].y = RADAR_CY;
        g_sweep_points[i][1].x = RADAR_CX + (int)(cosf(a) * RADAR_RADIUS); g_sweep_points[i][1].y = RADAR_CY + (int)(sinf(a) * RADAR_RADIUS);
        lv_line_set_points(g_sweep_lines[i], g_sweep_points[i], 2);
    }
    for (size_t i = 0; i < MAX_PLANES; ++i) {
        if (!g_target_active[i]) continue;
        if (g_target_emergency[i]) { lv_obj_set_style_opa(g_radar_targets[i], LV_OPA_COVER, 0); continue; }
        float d = angle_delta((float)g_sweep_deg, g_target_bearing[i]);
        lv_obj_set_style_opa(g_radar_targets[i], d < 6.0f ? LV_OPA_COVER : (d < 30.0f ? 220 : 165), 0);
    }
}

static void update_ui_data(const Summary &s, const AircraftDot *planes, size_t count)
{
    char buf[192], nid[10], nsq[5];
    ...
    snprintf(buf, sizeof(buf), "%d AIRCRAFT", s.aircraft); lv_label_set_text(g_overview_main, buf);
    lv_obj_set_style_text_color(g_overview_main, s.source_online ? lv_color_hex(0x50e050) : lv_color_hex(0xff4040), 0);
    snprintf(buf, sizeof(buf), "POS %d\nMSG/S %.0f\nMAX %.0f km", s.with_position, (double)s.msg_rate, (double)s.max_range_km); lv_label_set_text(g_overview_info, buf);
    snprintf(buf, sizeof(buf), "NEAREST %s\n%.1f km %d ft", nid, (double)s.nearest.distance_km, s.nearest.altitude_ft); lv_label_set_text(g_overview_near, buf);
    snprintf(buf, sizeof(buf), "%s\n%.1f km\n%d ft\n%.0f kt\nTRK %d\nSQ %s", nid, (double)s.nearest.distance_km, s.nearest.altitude_ft, (double)s.nearest.speed_kt, s.nearest.track, nsq); lv_label_set_text(g_nearest_info, buf);
    int64_t now = now_ms();
    ...
    snprintf(buf, sizeof(buf), "%s\nWi-Fi %s  MQTT %s\nSummary RX %ds\nAircraft RX %ds\nTargets %u",
             s.source_online ? "SOURCE ONLINE" : "SOURCE OFFLINE", g_wifi_connected ? "OK" : "DOWN", g_mqtt_connected ? "OK" : "DOWN", summary_age, aircraft_age, (unsigned)count);
    lv_label_set_text(g_status_info, buf);

    const float range = (float)g_radar_ranges[g_radar_range_idx];
    snprintf(buf, sizeof(buf), "PPI %d km", (int)range); lv_label_set_text(g_radar_title, buf);
    size_t shown = 0; if (count > MAX_PLANES) count = MAX_PLANES;
    for (size_t i = 0; i < MAX_PLANES; ++i) {
        g_target_active[i] = false; g_target_emergency[i] = false;
        if (i >= count || planes[i].distance_km <= 0 || planes[i].distance_km > range) { lv_obj_add_flag(g_radar_targets[i], LV_OBJ_FLAG_HIDDEN); continue; }
        float a = (planes[i].bearing - 90.0f) * PI_F / 180.0f;
        float rr = (planes[i].distance_km / range) * (RADAR_RADIUS - 10);
        int x = RADAR_CX + (int)lroundf(cosf(a) * rr), y = RADAR_CY + (int)lroundf(sinf(a) * rr);
        bool emergency = !strcmp(planes[i].squawk, "7500") || !strcmp(planes[i].squawk, "7600") || !strcmp(planes[i].squawk, "7700");
        int sz = emergency ? 10 : 8;
        lv_obj_set_size(g_radar_targets[i], sz, sz); lv_obj_set_style_bg_color(g_radar_targets[i], emergency ? lv_color_hex(0xff3030) : lv_color_hex(RADAR_GREEN), 0);
        lv_obj_set_pos(g_radar_targets[i], x - sz / 2, y - sz / 2); lv_obj_set_style_opa(g_radar_targets[i], emergency ? LV_OPA_COVER : 180, 0);
        lv_obj_remove_flag(g_radar_targets[i], LV_OBJ_FLAG_HIDDEN); lv_obj_move_foreground(g_radar_targets[i]);
        g_target_active[i] = true; g_target_bearing[i] = planes[i].bearing; g_target_emergency[i] = emergency; ++shown;
    }
    snprintf(buf, sizeof(buf), "%u/%u targets", (unsigned)shown, (unsigned)count); lv_label_set_text(g_radar_count, buf);
    lv_obj_move_foreground(g_radar_title); lv_obj_move_foreground(g_radar_count);
    ESP_LOGI(TAG, "PPI plotted %u/%u targets at range %dkm", (unsigned)shown, (unsigned)count, (int)range);
    if (g_alert[0] && now_ms() < g_alert_until_ms) { lv_label_set_text(g_alert_label, g_alert); lv_obj_remove_flag(g_alert_label, LV_OBJ_FLAG_HIDDEN); }
    else lv_obj_add_flag(g_alert_label, LV_OBJ_FLAG_HIDDEN);
}

static void ui_task(void *)
{
    Summary snapshot{}; AircraftDot planes[MAX_PLANES]{}; size_t count = 0; int last_page = -1; int64_t last_status_refresh = 0;
    while (true) {
        int64_t now = now_ms(); bool timed_status = (now - last_status_refresh >= 5000); bool need_data = g_data_dirty || timed_status;
        if (need_data && xSemaphoreTake(g_data_mutex, pdMS_TO_TICKS(50))) {
            snapshot = g_summary; count = g_plane_count > MAX_PLANES ? MAX_PLANES : g_plane_count; memcpy(planes, g_planes, sizeof(planes));
            xSemaphoreGive(g_data_mutex); g_data_dirty = false; if (timed_status) last_status_refresh = now;
        }
        bool sweep_due = (g_page == 1 && now - g_last_sweep_ms >= 55); bool page_changed = g_page != last_page;
        if (need_data || sweep_due || page_changed) {
            if (bsp_display_lock(250)) {
                ...
                if (sweep_due) { g_sweep_deg = (g_sweep_deg + 4) % 360; update_sweep(); g_last_sweep_ms = now_ms(); }
                bsp_display_unlock();
            }
        }
...
extern "C" void app_main(void)
{
    ...
    ESP_ERROR_CHECK(err);
    g_data_mutex = xSemaphoreCreateMutex(); ESP_ERROR_CHECK(g_data_mutex ? ESP_OK : ESP_ERR_NO_MEM);
    if (!prepare_mqtt()) ESP_LOGE(TAG, "MQTT preparation failed");
    ESP_LOGI(TAG, "Starting official ESP32-C3-LCDkit BSP display");
    g_display = bsp_display_start(); ESP_ERROR_CHECK(g_display ? ESP_OK : ESP_FAIL); ESP_ERROR_CHECK(bsp_display_backlight_on());
    if (bsp_display_lock(1000)) { build_ui(); bsp_display_unlock(); }
    xTaskCreate(ui_task, "adsb-ui", 4096, nullptr, 4, nullptr);
    ESP_LOGI(TAG, "Starting Wi-Fi"); wifi_init();
}
