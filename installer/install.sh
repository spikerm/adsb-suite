...
if curl -fL --retry 3 --connect-timeout 20 -o "$DB_TMP" "$DB_URL"; then
  gzip -t "$DB_TMP"; mv -f "$DB_TMP" "$DB_FILE"; chmod 644 "$DB_FILE"
else
  rm -f "$DB_TMP"
...
PY
  ...
  echo "Waarschuwing: /etc/default/readsb ontbreekt."
fi
...
if 'from v08_admin import register_admin' not in s:
...
if 'register_admin(app' not in s:
...
c.setdefault('receiver_lat',51.842837320295985); c.setdefault('receiver_lon',4.69044839675828)
...
if not c.get('admin_password_hash'):
    ...
    c['admin_password_hash']=f'pbkdf2_sha256${salt}${digest}'
    open('/run/adsb-suite-admin-password','w').write(password)
...
chmod 660 /etc/adsb-suite/config.json
